...
// Never let spawned CLI processes register fixtures in the developer's real
// ~/.gitnexus. The suite is sequential, so one isolated registry is sufficient.
const isolatedGitNexusHome = fs.mkdtempSync(path.join(os.tmpdir(), 'skills-e2e-home-'));

afterAll(() => {
  fs.rmSync(isolatedGitNexusHome, { recursive: true, force: true });
});
...
function runSkillsCli(cwd: string, timeoutMs = 45000) {
  return spawnSync(process.execPath, ['--import', tsxImportUrl, cliEntry, 'analyze', '--skills'], {
    ...
    env: {
      ...process.env,
      GITNEXUS_HOME: isolatedGitNexusHome,
 ...
 * 1. CLI exits 0
 * 2. .gitnexus/ exists
 * 3. >= minSkills SKILL.md files in direct gitnexus-generated-* skill directories
...
function assertSkillFiles(
  ...
  const skillDirs = fs
    ...
    .filter(
      (d) =>
        d.startsWith('gitnexus-generated-') && fs.statSync(path.join(skillsDir, d)).isDirectory(),
    );
  if (skillDirs.length === 0) {
    // Fixed GitNexus skills make .agents/skills exist even when community
    // generation is skipped, so absence must be checked by owned prefix.
    return false;
  }
...
/**
 * Assert AGENTS.md contains generated skill references and CLAUDE.md is not generated.
 * Automatically detects whether skills were generated by checking for
 * a gitnexus-generated-* skill directory.
 */
function assertContextFiles(result: ReturnType<typeof runSkillsCli>, tmpDir: string) {
  ...
  const skillsGenerated =
    ...
    fs.readdirSync(skillsDir).some((entry) => entry.startsWith('gitnexus-generated-'));
  ...
  if (skillsGenerated) {
    ...
    expect(agentsContent).toContain('.agents/skills/gitnexus-generated-');
  }
}
...
`,
  ...
  /**
   * Verify AGENTS.md is created and references generated skills.
   */
...
`,
   ...
   * and AGENTS.md still references generated skills.
   */
  it('second analyze --skills produces stable output', () => {
    ...
    /* AGENTS.md should still reference generated skills */
    const agentsPath = path.join(tmpDir, 'AGENTS.md');
    expect(fs.existsSync(agentsPath)).toBe(true);
    const agentsContent = fs.readFileSync(agentsPath, 'utf-8');
    expect(agentsContent).toContain('.agents/skills/gitnexus-generated-');
